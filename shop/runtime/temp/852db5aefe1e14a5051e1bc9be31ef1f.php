<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:63:"E:\PHP\tpshop\public/../application/home\view\order\create.html";i:1542167722;s:47:"E:\PHP\tpshop\application\home\view\layout.html";i:1542090549;}*/ ?>
<!DOCTYPE html>
<html>
...
<body>
					...
					<div class="shortcut">
						<ul class="fl">
							...
							<li class="f-item">请<a href="<?php echo url('home/login/login'); ?>">登录</a>
                                                       　<span><a href="<?php echo url('home/login/register'); ?>">免费注册</a></span></li>
                                                        <?php elseif(\think\Session::get('user_info.phone')): ?>
                                                        <li class="f-item">您好<a href="javaScript:;"><?php echo encrypt_phone(\think\Session::get('user_info.phone')); ?></a>
                                                       　<span><a href="<?php echo url('home/login/logout'); ?>" >退出</a></span></li>
                                                        <?php elseif(\think\Session::get('user_info.email')): ?>
                                                        <li class="f-item">您好<a href="javaScript:;"><?php echo \think\Session::get('user_info.email'); ?></a>
                                                       　<span><a href="<?php echo url('home/login/logout'); ?>" >退出</a></span></li>
                                                        <?php else: ?>
                                                        <li class="f-item">您好<a href="javaScript:;"><?php echo \think\Session::get('user_info.username'); ?></a>
                                                       　<span><a href="<?php echo url('home/login/logout'); ?>" >退出</a></span></li>
                                                        <?php endif; ?>
						</ul>
						<ul class="fr">
						...
						<div class="yui3-u Right shopArea">
							<div class="fr shopcar">
								<div class="show-shopcar" id="shopcar">
									<span class="car"></span>
									<a class="sui-btn btn-default btn-xlarge" href="/home/cart/index" target="_blank">
										<span>我的购物车</span>
					...
					<div class="yui3-g NavList">
						<div class="all-sorts-list">
							<div class="yui3-u Left all-sort">
                                                            <a href="<?php echo url('home/index/index'); ?>" style="color:white;text-decoration: none"><h4>全部商品分类</h4></a>
							</div>
							<div class="sort">
									<div class="all-sort-list2">
                                                                            <?php foreach($base_cate as $cate_one): ?>
										<div class="item bo">
	...
	<div class="cart py-container">
			...
			<div class="checkout-steps">
				...
				<div class="step-cont">
					<div class="addressInfo">
						<ul class="addr-detail" id='adress'>
						...
						<ul class="send-detail">
							<li>
								...
								<div class="sendGoods">
									...
									<ul class="yui3-g" cart_id='<?php echo $v['id']; ?>'>
										<li class="yui3-u-1-6">
		...
		<div class="clearfix trade">
			...
			<?php if($address): ?><div class="fc-receiverInfo">寄送至:<?php echo $address[0]['address']; ?> 收货人：<?php echo $address[0]['consignee']; ?> <?php echo $address[0]['phone']; ?></div><?php endif; ?>
		</div>
		<div class="submit">
			<a class="sui-btn btn-danger btn-xlarge" href="javascript:;">提交订单</a>
		</div>
	</div>
        <form class="sub_summart" method='post' action="<?php echo url('home/order/savepay'); ?>" style='display:none;'>
        ...
        <script>
            //给提交订单绑定提交时间
            $(function(){
                var address=$('#adress').find('div').length;
                var html=$('.modal-body').html();
                if(!address){
                    $('#adress').html(html);
                } 
                $('.btn-xlarge').click(function(){
                    //提交时需要提交的数据，cart_ids,pay_type,address_id,需要新增表单，用于提交
                    var address_id=$('.addr-item .selected').attr('address_id');
                    var pay_type=$('.payType').find('li.selected').attr('pay_type');
                    $('input[name=pay_type]').val(pay_type);
                    if(!address){
                        //当没有地址时，将地址信息改为字符串格式，并保存传递给后台，然后保存在address中
                        //{consignee:,address:,phone:}
                        var consignee=$('#adress').find('input[name=name]').val();
                        var address=$('#adress').find('input[name=address]').val();
                        var phone=$('#adress').find('input[name=phone]').val();
                        var str=JSON.stringify({'consignee':consignee,'address':address,'phone':+phone});
                        $('input[name=address_id]').attr('name','address').val(str); 
                    }else{
