<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:63:"E:\PHP\tpshop\public/../application/home\view\goods\detail.html";i:1541832230;s:47:"E:\PHP\tpshop\application\home\view\layout.html";i:1542178499;}*/ ?>
<!DOCTYPE html>
<html>
...
<body>
				...
				<div class="fl preview-wrap">
					...
					<div class="zoom">
						...
						<div id="preview" class="spec-preview">
							<span class="jqzoom"><img jqimg="<?php echo $pics_info[0]['pics_big']; ?>" src="<?php echo $pics_info[0]['pics_sma']; ?>" /></span>
						</div>
						<!--下方的缩略图-->
				...
				<div class="fr itemInfo-wrap">
					...
					<div class="clearfix choose">
						<div id="specification" class="summary-wrap clearfix">
                                                    <?php foreach($attr_info as $v): if($v['attr_type']==1): ?>
                                                    <dl>
								<dt>
									<div class="fl title">
										<i><?php echo $v['attr_name']; ?>:</i>
                                                                ...
                                                                <?php foreach($goodsattr_info[$v['id']] as $k=>$value): ?>
								<dd><a href="javascript:;" goods_attr_id="<?php echo $value['id']; ?>" class="<?php echo $k==0?'selected':''; ?>"><?php echo $value['attr_value']; ?><span title="点击取消选择">&nbsp;</span></a></dd>
								<?php endforeach; ?>
							</dl>
						...
						<div class="tab-content tab-wraped">
							<div id="one" class="tab-pane active">
								<ul class="goods-intro unstyled">
                                                                    <?php foreach($attr_info as $v): if($v['attr_type']==0): ?>
									<li><?php echo $v['attr_name']; ?>：<?php echo $goodsattr_info[$v['id']][0]['attr_value']; ?></li>
                                                                    <?php endif; endforeach; ?>
								</ul>
	...
	<script>
		//使用jQuery实现实现加号和减号，以及选择商品
		$(function(){

			//判定手动输入的数字是否正确
			$('#number').change(function(){
				var value=$(this).val();
				var value_pattern=/^[1-9]\d*$/g;
			...
			$('#mins').click(function(){
				var value=parseInt($('#number').val());	
				var mimnumber=$('#number').attr('minnum');
				if(isNaN(value)){
					alert('请输入数字');
					return;
				}
			...
			$('#specification').find('a').click(function(){
				...
				//当点击时，取出改行所有的a标签的所有class为selected属性
				$(this).closest('dl').find('a').removeClass('selected');
				//对选中的a标签添加selected属性
				$(this).addClass('selected');
			});	
                        
                        //点击加入购物车后，加入购物车，并提交表单
                        $('#addcart').click(function(){
                            //把number的值赋值给隐藏的表单的id
                            $('input[name=number]').val($('#number').val());
                            //获取goods_attr_id并拼接在一起
                            var select_a=$('a.selected');
                            var goods_attr_ids='';
                            $.each(select_a,function(i,v){
